...
//行  506
    ...
    {
        domain = "https://imazes.org";
        homeUrl = "https://imazes.org";
        updateUrl = "https://imazes.org";
    }
...
//        mainFrame.setIconImage(Toolkit.getDefaultToolkit().getImage(logoImg));
        mainFrame.setIconImage(Toolkit.getDefaultToolkit().getImage(getClass().getResource(logoImg)));

        initUI();
//        checkQuanxian();
        loadConfig();
        mainFrame.setTitle("FinalSpeed 最终版 by imazes");
        ...
        JButton button_fsa = createButton_Link("由 imazes修改","https://imazes.org");
        p5.add(button_fsa);
        JButton button_wlt = createButton_Link("仅供自用+测试","https://imazes.org");
        p5.add(button_wlt);
 ...
 //            trayIcon = new TrayIcon(Toolkit.getDefaultToolkit().getImage(offlineImg), name, trayMenu);
             trayIcon = new TrayIcon(Toolkit.getDefaultToolkit().getImage(getClass().getResource(logoImg)), name, trayMenu);
...
//                System.exit(0);
    ...
    void saveFile(byte[] data, String path) throws Exception {
        ...
        try {
            ...
            if (systemName.contains("windows")) {
                JOptionPane.showMessageDialog(null, "保存配置文件失败,请重新运行/以管理员身份运行! " + path);
                System.exit(0);
            }
            throw e;
        } finally {
